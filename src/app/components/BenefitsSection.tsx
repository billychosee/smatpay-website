...
import { FaHandshake, FaGlobeAmericas, FaUserCheck, FaMobileAlt } from "react-icons/fa";
...
const benefitsOfSmatpayItems = [
  ...
  {
    title: "Customers",
    description: "Access products on convenient debit order plans and build credit history",
    icon: FaUserCheck,
  },
  {
...
export default function BenefitsSection() {
  return (
    <section id="benefits" className="relative py-20 overflow-hidden bg-purple-50">
      {/* Animated background shapes */}
      <div className="absolute inset-0 overflow-hidden -z-10">
        {/* Large blurred circles */}
        <motion.div
          className="absolute w-72 h-72 rounded-full bg-[#8141D5]/20 top-10 left-10 blur-3xl"
          animate={{ x: [0, 50, 0], y: [0, -30, 0] }}
          transition={{ repeat: Infinity, duration: 25, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute w-64 h-64 rounded-xl bg-[#2f1991]/20 top-1/3 right-10 blur-2xl"
          animate={{ x: [0, -40, 0], y: [0, 20, 0] }}
          transition={{ repeat: Infinity, duration: 20, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute w-56 h-56 rounded-full bg-[#8141D5]/15 bottom-10 left-1/4 blur-2xl"
          animate={{ x: [0, 30, 0], y: [0, -20, 0] }}
          transition={{ repeat: Infinity, duration: 30, ease: "easeInOut" }}
        />

        {/* Floating payment icons */}
        {benefitsOfSmatpayItems.map((_, i) => (
          <motion.div
            key={i}
          ...
          >
            <FaHandshake className="w-6 h-6 text-[#8141D5]/60" />
          </motion.div>
        ))}
      </div>
      ...
      <div className="container relative z-10 px-6 mx-auto text-center max-w-7xl">
        {/* Heading */}
        <p className="text-sm font-semibold tracking-widest text-[#8141D5] uppercase">Benefits of SmatPay</p>
        <h3 className="mt-2 text-3xl font-extrabold text-[#2f1991] sm:text-4xl">
          Why Our Customers Love SmatPay
        </h3>
        <p className="max-w-2xl mx-auto mt-4 text-gray-700 sm:text-lg">
          Discover how SmatPay simplifies payments, expands market reach, and empowers businesses to grow seamlessly.
        </p>
        ...
        <div className="grid gap-8 mt-12 sm:grid-cols-2 lg:grid-cols-4">
          {benefitsOfSmatpayItems.map((item, i) => (
            ...
            >
              <div className="flex items-center justify-center w-16 h-16 mb-4 bg-[#8141D5]/20 rounded-full">
