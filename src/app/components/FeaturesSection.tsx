  ...
  return (
    <section id="features" className="relative py-24 overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden -z-10" aria-hidden="true">
        ...
        <motion.div
          className="absolute w-80 h-80 rounded-full bg-[#8141D5]/70 top-12 left-12 blur-3xl shadow-[0_0_60px_20px_rgba(129,65,213,0.65)]"
          animate={{ x: [0, 80, -20, 0], y: [0, -40, 20, 0] }}
          transition={{ repeat: Infinity, duration: 28, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute w-72 h-72 rounded-xl bg-[#2f1991]/50 top-1/3 right-12 blur-2xl shadow-[0_0_40px_14px_rgba(47,25,145,0.6)]"
          animate={{ x: [0, -60, 0], y: [0, 40, 0] }}
          transition={{ repeat: Infinity, duration: 22, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute w-64 h-64 rounded-full bg-[#8141D5]/40 bottom-8 left-1/4 blur-2xl shadow-[0_0_60px_20px_rgba(129,65,213,0.5)]"
        ...
        {[...Array(6)].map((_, i) => (
          <motion.div
            ...
            style={{
              filter: "drop-shadow(0 6px 14px rgba(129,65,213,0.8))",
            }}
            animate={{
          ...
          >
            {i % 2 === 0 ? (
              <FaCreditCard className="w-7 h-7 text-[#EAD7FF]" />
            ) : (
              <FaDollarSign className="w-7 h-7 text-[#C1B3FF]" />
            )}
          </motion.div>
        ))}
      </div>
      ...
      <div className="container relative z-10 px-6 mx-auto text-center max-w-7xl">
        ...
        <div className="grid gap-6 mt-10 sm:grid-cols-2 lg:grid-cols-3">
          {keyFeaturesItems.map((item, i) => (
            <div
              ...
              style={{
                // Semi-transparent white with purple tint
                background: "linear-gradient(to bottom right, rgba(255,255,255,0.88), rgba(245,243,255,0.75))",
                border: "1px solid rgba(99,102,241,0.25)", // subtle purple border
              }}
            >
              ...
              <span
                ...
                style={{
                  background: "radial-gradient(circle at 20% 20%, rgba(129,65,213,0.25), rgba(0,0,0,0) 40%)",
                  filter: "blur(8px)",
              ...
              <div className="pointer-events-none absolute -right-6 -top-6 h-28 w-28 rounded-full bg-[#2f1991]/10 blur-2xl" aria-hidden="true" />
            </div>
          ))}
        </div>
        ...
        <div className="mt-10">
          <Link
            href="/features"
            className="inline-flex rounded-full bg-white px-8 py-3 text-sm font-bold text-[#2f1991] ring-1 ring-[#2f1991] transition hover:bg-[#2f1991] hover:text-white"
          >
            See All
          </Link>
        </div>
