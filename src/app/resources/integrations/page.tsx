...
const integrationsSchema = {
  ...
  description:
    "Explore the various platforms and services that integrate seamlessly with SmatPay's payment gateway. Connect with popular e-commerce platforms and APIs.",
      ...
      <div>
        ...
        <div className="relative w-full h-[500px] md:h-[600px] flex flex-col items-center justify-center text-center text-white overflow-hidden">
          ...
          <motion.div className="relative z-10 max-w-2xl px-4" variants={fadeInUp} initial="hidden" animate="visible">
            <h1 className="mb-6 text-4xl font-bold leading-tight sm:text-5xl md:text-4xl">Integrations</h1>
            ...
            <a href="http://doc.smatpay.africa/" target="_blank" rel="noopener noreferrer" className="inline-block px-8 py-3 font-semibold text-white transition bg-purple-600 rounded-full hover:bg-purple-700">
              Explore API Docs
            </a>
          </motion.div>
        </div>
...
>
  <div className="container grid gap-16 px-4 mx-auto md:grid-cols-2 md:items-stretch">
    ...
    >
      <h2 className="mb-6 text-4xl font-bold text-[#2f1991]">
        Why Integrate with SmatPay?
      </h2>
    ...
    >
      ...
      <div className="relative z-10 h-full overflow-hidden transition rounded-lg shadow-2xl hover:scale-105">
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
              {[
                ...
                <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: i * 0.2 }} viewport={{ once: true }} whileHover={{ scale: 1.05 }} className="flex flex-col items-center p-6 transition bg-white rounded-lg shadow-md hover:shadow-xl">
                  <div className="flex items-center justify-center w-24 h-24 mb-4 text-5xl text-[#2f1991] rounded-full bg-gray-100"><item.icon /></div>
        ...
        <motion.section className="relative py-20 bg-[#2f1991] overflow-hidden" style={{ backgroundImage: "url('/shopping-cart.jpg')", backgroundSize: 'cover', backgroundPosition: 'center', backgroundBlendMode: 'overlay' }} initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }} viewport={{ once: true, amount: 0.4 }}>
          <div className="absolute inset-0 bg-[#2f1991] opacity-70"></div>
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
              {KeyIntegrationComponents.map((item, index) => (
                <motion.div key={index} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.2 }} viewport={{ once: true }} whileHover={{ scale: 1.05 }} className="flex flex-col items-center p-8 text-center transition bg-white rounded-lg shadow-lg hover:shadow-xl">
                  <div className="flex items-center justify-center w-20 h-20 mb-4 bg-gray-100 rounded-full"><item.icon className="w-10 h-10 text-[#2f1991]" /></div>
          ...
          <div className="container relative z-10 px-5 mx-auto text-center md:px-24">
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                { icon: FaLayerGroup, color: 'text-purple-500 bg-purple-100', title: 'Integration Review', desc: 'Comprehensive review post-integration.' },
              ].map((item, i) => (
        ...
        <motion.section className="py-20 bg-white" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }} viewport={{ once: true, amount: 0.2 }}>
          <div className="container grid gap-8 px-4 mx-auto md:grid-cols-2">
            ...
            <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }} viewport={{ once: true }} className="flex flex-col items-center justify-center text-center md:items-start md:text-left">
              ...
              <p className="mb-6 text-gray-600">Streamline payments, secure transactions, and boost conversions with SmatPay integration.</p>
              <a href="http://doc.smatpay.africa/" target="_blank" rel="noopener noreferrer" className="px-8 py-3 font-semibold text-white transition bg-purple-600 rounded-full hover:bg-purple-700">
